\chapter{Example chapter}
{
    ...
    A scientific thesis requires concise and precise language. Expressions like \emph{high temperature}, \emph{good rheology} or every-day language should be avoided. 

    Using past tense is reasonable for experiments conducted by yourself or other researchers in literature. General results should be reported in present tense, as they are valid at any time.

    Use consistent writing (British English, American English) throughout the text and whenever multiple spellings of a word are possible. 
    ...
    Make sure that the image is readable, i.e. features are large enough and the image quality is high. For pictures, use \texttt{JPEG} format and include a scale bar, if the size is relevant (e.g. microscopy images).  For diagrams and sketches use vector formats like \texttt{PDF} or \texttt{SVG} for optimal quality. For diagrams, make sure to include axis labels with a sufficiently large font and use \emph{in <unit>} instead of rectangular brackets to indicate units. Make sure that colors are easy to distinguish, some background on colors may be found \href{here}{https://colorbrewer2.org} and \href{here}{https://bids.github.io/colormap/}. 
